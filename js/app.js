    ...
    function spollers() {
        ...
        if (spollersArray.length > 0) {
            ...
            function initSpollerBody(spollersBlock, hideSpollerBody = true) {
                let spollerItems = spollersBlock.querySelectorAll("[data-spoller]");
                if (spollerItems.length) spollerItems.forEach((spollerItem => {
                    let spollerTitle = spollerItem.querySelector("[data-sp-button]");
                    if (hideSpollerBody) {
            ...
            function setSpollerAction(e) {
                const el = e.target;
                if (el.closest("[data-sp-button]") && el.closest("[data-spollers]")) {
                    e.preventDefault();
                    if (el.closest("[data-spollers]").classList.contains("_spoller-init")) {
                        const spollerTitle = el.closest("[data-sp-button]");
            ...
            function hideSpollersBody(spollersBlock) {
                const spollerActiveBlock = spollersBlock.querySelector("[data-spoller][open]");
                if (spollerActiveBlock && !spollersBlock.querySelectorAll("._slide").length) {
                    const spollerActiveTitle = spollerActiveBlock.querySelector("[data-sp-button]");
                    const spollerSpeed = spollersBlock.dataset.spollersSpeed ? parseInt(spollersBlock.dataset.spollersSpeed) : 500;
    ...
    document.addEventListener("DOMContentLoaded", (() => {
        document.addEventListener("click", (e => {
            ...
            if (isMobile.any()) {
                ...
                if (buttons.length) if (target && target.closest("[data-hover]")) {
                    if (target.closest("[data-spoller]") && screenWidth < 767.98) return;
                    target.closest("[data-hover]").parentNode.classList.toggle("_hover");
                } else if (!target.closest("._hover") && document.querySelectorAll("._hover").length > 0) buttons.forEach((button => {
